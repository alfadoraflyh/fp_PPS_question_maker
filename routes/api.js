...
router.post('/topik', (req, res) => {
    const { nama } = req.body
    const sqlInsert = `INSERT INTO topik (id_topik, nama_topik) VALUES (NULL, ?)`
...
router.delete('/topik/:id', (req, res) => {
    ...
    const sqlSearch = `SELECT * FROM topik WHERE id_topik = ?`
    const sqlDelete = `DELETE FROM topik WHERE id_topik = ?`
...
router.get('/soal', (req, res) => {
    const sqlSoal = `SELECT soal.id, soal.pertanyaan, soal.tingkat_kesulitan, topik.nama_topik FROM soal INNER JOIN topik ON soal.id_topik = topik.id_topik`
...
router.get('/paketsoal/:id', (req, res) => {
    const id = req.params.id
    const sqlPaketsoal = `SELECT * FROM paketsoal WHERE id_paket_soal = ?`
    const sqlSoalujian = `SELECT soal.pertanyaan, soal.pilihan_a, soal.pilihan_b, soal.pilihan_c, soal.pilihan_d FROM soal_ujian INNER JOIN soal ON soal_ujian.id_soal = soal.id WHERE soal_ujian.id_paket_soal = ?`
...
router.get('/konfigurasi/:id', (req, res) => {
    const id = req.params.id
    const sqlPaketsoal = `SELECT id_paket_soal, nama_paket_soal, jumlah_soal FROM paketsoal WHERE id_paket_soal = ?`
    const sqlTopikujian = `SELECT topik_ujian.id_topik_ujian, topik.nama_topik, topik_ujian.persentase_topik, topik_ujian.persentase_mudah, topik_ujian.persentase_sedang, topik_ujian.persentase_sulit FROM topik_ujian INNER JOIN topik ON topik_ujian.id_topik = topik.id_topik WHERE topik_ujian.id_paket_soal = ?`
...
router.post('/konfigurasi', (req, res) => {
    ...
    const sqlInsert = `INSERT INTO paketsoal (id_paket_soal, nama_paket_soal, jumlah_soal) VALUES (NULL, ?, ?, NULL)`
    
    db.query(sqlInsert, [ nama_paket_soal, jumlah_soal ], (error, result) => {
        if (error) {
...
router.put('/konfigurasi/:id', (req, res) => {
    const id_paket_soal = req.params.id
    const { nama_paket_soal, jumlah_soal } = req.body
    const sqlSearch = `SELECT id_paket_soal FROM paketsoal WHERE id_paket_soal = ?`
    const sqlUpdate = `UPDATE paketsoal SET nama_paket_soal = ?, jumlah_soal = ? WHERE id_paket_soal = ?`
    ...
    db.query(sqlSearch, [ id_paket_soal ], (error, result) => {
        ...
        if (result.length) {
            db.query(sqlUpdate, [ nama_paket_soal, jumlah_soal, id_paket_soal], (error, result) => {
                if (error) {
...
router.post('/topikujian', (req,res) => {
    const { id_paket_soal, id_topik, persentase_topik, persentase_mudah, persentase_sedang, persentase_sulit } = req.body

    if (parseFloat(persentase_mudah) + parseFloat(persentase_sedang) + parseFloat(persentase_sulit) == 1.00) {
        const sqlCheck = `SELECT EXISTS (SELECT id_topik_ujian FROM topik_ujian WHERE id_paket_soal = ? AND id_topik = ?) AS isExists`
        const sqlInsert = `INSERT INTO topik_ujian (id_topik_ujian, id_paket_soal, id_topik, persentase_topik, persentase_mudah, persentase_sedang, persentase_sulit) VALUES (NULL, ?, ?, ?, ?, ?, ?)`
        db.query(sqlCheck, [ id_paket_soal, id_topik ], (error, result1) => {
            if (error) {
                return res.status(500).json({ message: 'Ada kesalahan!' })
            }

            if (result1[0].isExists == 0) {
                db.query(sqlInsert, [ id_paket_soal, id_topik, persentase_topik, persentase_mudah, persentase_sedang, persentase_sulit ], (error, result2) => {
                    if (error) {
...
router.delete('/topikujian/:id', (req, res) => {
    const id_topik_ujian = req.params.id
    const sqlSearch = `SELECT * FROM topik_ujian WHERE id_topik_ujian = ?`
    const sqlDelete = `DELETE FROM topik_ujian WHERE id_topik_ujian = ?`
    db.query(sqlSearch, [ id_topik_ujian ], (error, result) => {
        if (error) {
            return res.status(500).json({ message: 'Ada kesalahan!' })
        }
        
        if (result.length) {
            db.query(sqlDelete, [ id_topik_ujian ], (error, result) => {
                if (error) {
...
router.post('/paketsoal/:id', (req, res) => {
    ...
    async function checkConfig(id) {
        try {
            const sqlCheckConfig = `SELECT ROUND(SUM(persentase_topik), 2) AS 'total' FROM topik_ujian WHERE id_paket_soal = ?`
    ...
    async function getNumofSoal(id) {
        try {
            const sqlPaketsoal = `SELECT jumlah_soal FROM paketsoal WHERE id_paket_soal = ?`
            ...
            const sqlTopikujian = `SELECT id_topik, ROUND((persentase_mudah * persentase_topik * ${ jumlahsoal }), 0) AS 'soal_mudah', ROUND((persentase_sedang * persentase_topik * ${ jumlahsoal }), 0) AS 'soal_sedang', ROUND((persentase_sulit * persentase_topik * ${ jumlahsoal }), 0) AS 'soal_sulit' FROM topik_ujian WHERE id_paket_soal = ?`
        ...
        try {
            ...
            if (totalPersenTopik == 1.00) {
                ...
                if (status == 'tersedia') {
                    ...
                    const sqlInsert = "INSERT INTO soal_ujian (id_soal_ujian, id_paket_soal, id_soal) VALUES " + values
